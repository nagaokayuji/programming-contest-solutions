...
void _solve() {
  ...
  Int y1max = -INF;
  each(xy, xy90) {
    chmin(x1min, xy.first);
    chmin(y1min, xy.second);
    chmax(x1max, xy.first);
    chmax(y1max, xy.second);
  }
  ...
  rep(_, Q) {
    ...
    Int y = xy90[q].second;
    Int ret =
        max({abs(x - x1min), abs(x - x1max), abs(y - y1min), abs(y - y1max)});
    cout << ret << "\n";
  }
}
