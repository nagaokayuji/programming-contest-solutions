...
void _solve() {
  ...
  V<V<Int>> dp(1 << N, V<Int>(K + 1, LINF));
  dp[0][0] = 0;
  for (int k = 1; k <= K; k++) {
    rep(selected_bit, 1 << N) {
      for (int subset = selected_bit; subset > 0;
           subset = selected_bit & (subset - 1)) {
        chmin(dp[selected_bit][k],
              max(dp[selected_bit ^ subset][k - 1], ds[subset]));
      }
    }
  }
