...
void _solve() {
  ...
  range(i, 2 * N) {
    range(j, 2 * N - i - 1) {
      ...
      cin >> k;
      int to = i + j + 1;
      aisho[i][to] = aisho[to][i] = k;
    }
  }

  int ans = 0;

  auto dfs = [&](auto self, int xorsum = 0, int i = 0,
                 int matched = 0) -> void {
    if (matched == ((1 << (2 * N)) - 1)) {
      chmax(ans, xorsum);
    }
    ...
    range(j, i + 1, 2 * N) {
      if (bit(matched, i) || bit(matched, j)) continue;

      int nxt_matched = matched | (1 << i) | (1 << j);
      int to_i = 0;

      range(k, 2 * N) {
        if (!bit(nxt_matched, k)) {
          to_i = k;
          break;
        }
      }

      self(self, xorsum ^ aisho[i][j], to_i, nxt_matched);
    }
  };
  dfs(dfs);
  cout << ans << endl;
