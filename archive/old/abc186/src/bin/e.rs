...
fn solve(n: i64, s: i64, k: i64) {
    ...
    if d == 1 {
        ...
        let m = n / d;
        let inv = mod_inverse(a / d, m);
        println!("{}", (inv * (b / d)) % m);
        return;
    }

    // let inv = mod_inverse(k, n);
    // println!("{}", inv * (n - s));
}
...
pub mod internal_math {
    ...
    mod tests {
        ...
        use internal_math::{inv_gcd, is_prime, pow_mod, primitive_root, safe_mod, Barrett};
