...
fn main() {
    ...
    let mut fact = vec![Mint::new(1); MX];
    let mut a = a.iter().map(|&x| Mint::new(x)).collect::<Vec<_>>();
    for i in 2..MX {
        fact[i] = fact[i - 1] * Mint::new(i);
    }

    let mut d = vec![Mint::new(0); k + 1];
    let mut ma = vec![Mint::new(1); n];
    for i in 0..=k {
    ...
    for x in 1..=k {
        let mut sub = 0i64;
        for i in 0..n {
            sub += mod_pow(2 * a[i].val() as i64, x as i64, MOD);
            sub %= MOD;
        }
        let sub = Mint::new(sub);
        // dbg!(&sub);
        let mut ad = Mint::new(0);
        let mut ret = fact[x];
        for l in 0..=x {
            ad = ad + d[l] * d[x - l];
        }
