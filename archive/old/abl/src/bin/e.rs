...
fn main() {
    input! {//
        n:usize,q:usize,
        lrd:[(Usize1,Usize1,i32);q],
    }
    let mut sg = LazySegtree::<F>::new(n);
    for i in 0..n {
        sg.set(i, (1.into(), 10.into()));
    }
    // sg.set(0, (Mint::new(1), Mint::new(1)));
    for &(l, r, d) in lrd.iter() {
        sg.apply_range(l, r + 1, d.into());
        // dbg!(&sg.all_prod());
        println!("{}", sg.all_prod().0);
    }
    // dbg!(Mint::new(3232) * Mint::new(123));
}
struct F;
struct M;
impl Monoid for M {
    ...
    fn binary_operation(&(a, b): &Self::S, &(c, d): &Self::S) -> Self::S {
        (
            // a * (Mint::new(9) * d + Mint::new(1)) + c,
            // b * (Mint::new(9) * d + Mint::new(1)) + d,
            // a + c,
            // b + d,
            a * d + c,
            b * d,
...
impl MapMonoid for F {
    ...
    fn mapping(&f: &Self::F, &x: &<Self::M as Monoid>::S) -> <M as Monoid>::S {
        if f == Mint::new(0) {
            x
        } else {
            ((x.1 - Mint::new(1)) / Mint::new(9) * f, x.1)
        }
    }
