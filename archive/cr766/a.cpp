...
void solve() {
    ...
    FOR(i, 0, n - 1) { cin >> field[i]; }
    deque<tuple<int, int, int>> deq;
    FOR(i, 0, n - 1) FOR(j, 0, m - 1) {
        if (field[i][j] == 'B') {
            deq.emplace_back(make_tuple(i, j, 0));
        }
    }
    auto dp = vector<vector<int>>(n, vector<int>(m, INF));
    while (len(deq) > 0) {
        auto [nowi, nowj, nowc] = deq.front();
        chmin(dp[nowi][nowj], nowc);
        deq.pop_front();

        FOR(i, 0, n - 1) {
            if (chmin(dp[i][nowj], nowc + 1)) {
                deq.emplace_back(i, nowj, nowc + 1);
            }
        }
        FOR(j, 0, m - 1) {
            if (chmin(dp[nowi][j], nowc + 1)) {
                deq.emplace_back(nowi, j, nowc + 1);
            }
        }
    }
    int ans = dp[r][c];
