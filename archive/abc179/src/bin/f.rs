...
fn main() {
    ...
    row.insert(n - 1, n - 2);
    col.insert(n - 1, n - 2);
    for &(qq, x) in query.iter() {
        match qq {
            1 => {
                let a = col.range(x..).next().unwrap();
                ans -= a.1;
                let mut k = row.entry(*(a.1)).or_insert(n - 1);
                *k = min(*k, x - 1);
            }
            _ => {
                let a = row.range(x..).next().unwrap();
                ans -= a.1;
                let mut k = col.entry(*(a.1)).or_insert(n - 1);
                *k = min(*k, x - 1);
            }
        };
    }
    let mut k = ModInt1000000007::new(3);
    println!("{}", ans);
}
