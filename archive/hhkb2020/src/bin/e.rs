...
fn main() {
    ...
    for i in 0..h {
        for j in 0..w {
            if s[i][j] == '.' {
                // dbg!((i, j));
                row[i][j] = if j > 0 { row[i][j - 1] } else { 0 } + 1;
            }
        }
        for j in (0..w).rev() {
            if row[i][j] > 0 && j + 1 < w && row[i][j + 1] > 0 {
                row[i][j] = row[i][j + 1];
            }
        }
    }
    for j in 0..w {
        for i in 0..h {
            if s[i][j] == '.' {
                col[i][j] = if i > 0 { col[i - 1][j] } else { 0 } + 1;
            }
        }
        for i in (0..h).rev() {
            if col[i][j] > 0 && i + 1 < h && col[i + 1][j] > 0 {
                col[i][j] = col[i + 1][j];
            }
        }
    }
    ...
    // ans = Mint::new(2).pow(cnt as u64);

    for i in 0..h {
        for j in 0..w {
            if s[i][j] == '.' {
                // dbg!(row[i][j] + col[i][j] - 1);
                let kosu = row[i][j] + col[i][j] - 1;
                let tmp = Mint::new(2).pow(kosu as u64) - Mint::new(1);
                let tmp2 = Mint::new(2).pow((cnt - kosu) as u64);
                // dbg!(tmp * tmp2);
                ans += tmp * tmp2;
            }
        }
