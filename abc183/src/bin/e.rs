...
fn main() {
    ...
    dp[0][0] = 1.into();
    let mut dpsr = vec![vec![Mint::new(0); w + 1]; h + 1];
    let mut dpsc = vec![vec![Mint::new(0); w + 1]; h + 1];
    let mut dpsd = vec![vec![Mint::new(0); w + 1]; h + 1];
    for i in 0..h {
        for j in 0..w {
            if s[i][j] == '#' {
                continue;
            }
            dpsr[i][j + 1] = dpsr[i][j] + dp[i][j];
            dpsc[i + 1][j] = dpsc[i][j] + dp[i][j];
            dpsd[i + 1][j + 1] = dpsd[i][j] + dp[i][j];
            if i + 1 < h {
                dp[i + 1][j] += dpsc[i + 1][j];
            }
            if j + 1 < w {
