...
void _solve() {
  ...
  rep(i, M) {
    ...
    cin >> a >> b;
    a--, b--;
    g[a].emplace_back(max(0, H[b] - H[a]), b);
    g[b].emplace_back(max(0, H[a] - H[b]), a);
  }
  dbg(g);

  using P = pair<Int, int>;
  auto dijkstra = [](V<V<P>> g, int start = 0) {
    ...
    while (!pq.empty()) {
      auto [d, v] = pq.top();
      pq.pop();
      if (dist[v] != d) continue;     // 必要なやつ
      for (auto [cost, nx] : g[v]) {  // pair の中身による
        Int nd = d + cost;
        if (chmin(dist[nx], nd)) {
          pq.emplace(nd, nx);
        }
  ...
  dbg(dist);
  Int ans = 0;
  rep(i, N) { chmax(ans, -(H[i] - H[0] + dist[i])); }
  cout << ans << endl;
}
