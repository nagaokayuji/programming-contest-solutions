...
fn main() {
    ...
    let mut dp = vec![vec![vec![vec![Mint::new(0); 2]; 2]; 2]; 63];
    dp[60][0][0][0] = 1.into();
    for bit in (0..60).rev() {
        let lb = l >> bit & 1;
        let rb = r >> bit & 1;
        for ok_l in 0..=1 {
            for ok_r in 0..=1 {
                for same_lr_len in 0..=1 {
                    let pre = dp[bit + 1][ok_l][ok_r][same_lr_len];
                    for x in 0..=1 {
                        for y in 0..=1 {
                            let mut next_same = same_lr_len;
                            let mut next_ok_l = ok_l;
                            let mut next_ok_r = ok_r;

                            if (x & !y) > 0 {
                                continue;
                            }
                            // 次
                            if same_lr_len == 0 && x != y {
                                continue;
                            }
                            if x == 1 && y == 1 {
                                next_same = 1;
                            ...
                            dp[bit][next_ok_l][next_ok_r][next_same] += pre;
                        }
                    }
                }
            }
