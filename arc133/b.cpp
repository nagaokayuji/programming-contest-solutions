...
void _solve() {
  ...
  each(p, P) {
    V<int> cango;
    range(q, p, N + 1, p) { cango.push_back(Qposs[q]); }
    sort(all(cango), [&](int a, int b) { return a > b; });
    each(x, cango) l.push_back(x);
  }
  ...
  range(i, 0, l.size()) {
    int val = l[i];
    int lb = lower_bound(all(dp), val) - dp.begin();
    chmin(dp[lb], val);
  }
  auto a = lower_bound(all(dp), INF) - dp.begin();
  cout << a << endl;
}
